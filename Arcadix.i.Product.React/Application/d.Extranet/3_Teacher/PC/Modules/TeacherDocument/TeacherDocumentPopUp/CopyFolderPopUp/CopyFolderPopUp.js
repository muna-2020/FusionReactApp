...
const TeacherCopyFolderPopUp = props => {
    ...
    return (
        <div className="copying-folder-wrapper-class">
            <span className="copying-header">{Localization.TextFormatter(objTextResource, 'MoveFolderPopUpHeadeer')}</span>
            ...
            <div className="footer-block">
                ...
                <button className="button yellow-button" onClick={() => { OnClickSave() }}>{Localization.TextFormatter(objTextResource, 'Save')}</button>
            </div>
        </div>
    );
...
TeacherCopyFolderPopUp.DynamicStyles = () => {
    return [
        ...
        "/Css/Application/2_School/ReactJs/PC/Modules/SchoolDocument/SchoolDocumentPopUp/CopyingFolderPopUp.css"
    ];
};
