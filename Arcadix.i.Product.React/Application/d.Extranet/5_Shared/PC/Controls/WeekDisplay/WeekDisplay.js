...
import AngleLeftBoldImage from '@inlineimage/Application/d.Extranet/5_Shared/PC/angle_left_bold.svg?inline';
import AngleRightBoldImage from '@inlineimage/Application/d.Extranet/5_Shared/PC/angle_right_bold.svg?inline';
...
const WeekDisplay = props => {
    ...
    */
    function GetContent() {
        ...
        let strStartDate = objContext.WeekDisplay_ModuleProcessor.GetLoacleDate(objContext, state.StartDate);
        let strEndDate = objContext.WeekDisplay_ModuleProcessor.GetLoacleDate(objContext, state.EndDate);
        return (
            <div className={props.IsFromPupil ? "pupil-week-display" : "week-display"}>
                <div className="slider-flex">
                    {
                        !state.isLeftEnd && (props.DisableIcon == undefined || !props.DisableIcon) ?
                            <button className="slider slide-left" style={{ backgroundColor: props.backgroundColor }} onClick={() => { objContext.WeekDisplay_ModuleProcessor.GetDateRangeForDisplay(objContext, true, false); }}>
                                <img src={AngleLeftBoldImage} alt="" />
                            </button> :
                    ...
                    {
                        !state.isRightEnd && (props.DisableIcon == undefined || !props.DisableIcon) ?
                            <button className="slider slide-right" style={{ backgroundColor: props.backgroundColor }} onClick={() => { objContext.WeekDisplay_ModuleProcessor.GetDateRangeForDisplay(objContext, false, true); }} >
                                <img src={AngleRightBoldImage} alt="" />
                            </button> :
...
*/
export default connect(ExtranetBase_Hook.MapStoreToProps(WeekDisplay_ModuleProcessor.StoreMapList()))(WeekDisplay);
