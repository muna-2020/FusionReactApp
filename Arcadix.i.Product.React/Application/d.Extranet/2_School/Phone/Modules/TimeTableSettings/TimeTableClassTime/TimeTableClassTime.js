...
import * as TimeTableClassTime_Hook from '@shared/Application/d.Extranet/2_School/Phone/Modules/TimeTableSettings/TimeTableClassTime/TimeTableClassTime_Hook';
import TimeTableClassTime_ModuleProcessor from '@shared/Application/d.Extranet/2_School/Phone/Modules/TimeTableSettings/TimeTableClassTime/TimeTableClassTime_ModuleProcessor';
        ...
        if (arrTimeTableSlots.length > 0) {
            ...
            if (!objValidateResultRef.blnIsDataValid) {
                ...
                objValidateResultRef["arrErrors"].forEach(objError => {
                    ...
                    objError.objErrorAtId.current.setAttribute("style", "border: 3px solid black;"); // "outline:focus"
                });
            ...
            <React.Fragment>
                <div className={'time-table-section'}>
                    <div className="table-heading">
                        <span>{Localization.TextFormatter(objTextResource, 'LessonText')}</span>
                    </div>

                    <div className="control-buttons">
                        <div className="control-buttons-left">
                            <button onClick={() => { objContext.TimeTableClassTime_ModuleProcessor.HandleAddRow(objContext); }}>
                                <img src={imgPlus} />
                                <span>{Localization.TextFormatter(objTextResource, 'AddButtonText')}</span>
                            </button>
                        </div>
                        <div className="control-buttons-right">
                            <button className="button-white" onClick={() => { objContext.TimeTableClassTime_ModuleProcessor.HandleReset(objContext); }}>
                                <span>{Localization.TextFormatter(objTextResource, 'ResetButtonText')}</span>
                            </button>
                            <button className="button-brown" onClick={(event) => { HandleSave(event) }} >
