...
import * as LearnProfile_Hook from '@shared/Application/d.Extranet/4_Pupil/PC/Modules/LearnProfile/LearnProfile_Hook';
import LearnProfile_ModuleProcessor from '@shared/Application/d.Extranet/4_Pupil/PC/Modules/LearnProfile/LearnProfile_ModuleProcessor';

//Components used in module.
import ChartOne from '@root/Application/d.Extranet/4_Pupil/PC/Modules/LearnProfile/ChartOne';
import WeekDisplay from "@root/Application/d.Extranet/5_Shared/PC/Controls/WeekDisplay/WeekDisplay";

/**
* @name LearnProfile
* @param {object} props props
* @summary This component displays the LearnProfile data.
* @returns {object} div that encapsulated the components with LearnProfile details.
*/
const LearnProfile = (props) => {
    ...
    */
    const [state, dispatch] = useReducer(ExtranetBase_Hook.Reducer, LearnProfile_Hook.GetInitialState(props));
    ...
    */
    let objContext = { state, props, dispatch, ["ModuleName"]: "LearnProfile", ["LearnProfile_ModuleProcessor"]: new LearnProfile_ModuleProcessor(), Object_Framework_Services_TextResource };
     ...
     */
    objContext.LearnProfile_ModuleProcessor.Initialize(objContext, objContext.LearnProfile_ModuleProcessor);

    /**
    * @name Initialize
    * @param {object} objContext context object
    * @summary Initialize method call in LearnProfile_Hook, that contains all the custom hooks.
    * @returns null
    */
    LearnProfile_Hook.Initialize(objContext);
    ...
    */
    const CalculateHeightsAndWidthsTemp = (arrModifiedSubjectData) => {
        ...
        let intTestLabelHeight = 25;
        arrModifiedSubjectData.forEach((objSubject, intSubjectIndex) => {
            let tbl = document.getElementById('tbl_' + objSubject['SubjectId']);

            //if (tbl) {
            //    state.intCompetencyInterval = ((tbl.offsetWidth - 245) / (state.blnShowSixCompetencies ? 7 : 8));
            ...
            objSubject["SubSubjects"].forEach(objSubSubject => {
                ...
                if (arrResults.length > 0) {
                    ...
                    let arrOrientationSubSubjectLines = objSubSubject["Lines"];
                    arrOrientationSubSubjectLines.forEach((objLine, index) => {
                        let line = lineRefs.current[index]; //document.getElementById('line_' + objLine['uLineId']);
                        let objFirstTest = arrResults.filter(x => x["uTestTokenId"] == objLine["uTestTokenId1"])[0];
                        let objSecondTest = arrResults.filter(x => x["uTestTokenId"] == objLine["uTestTokenId2"])[0];
                        if (line) {
                            line.setAttribute('x1', objFirstTest["x"]);
            ...
            //let svgImage = document.getElementById('svg_' + objSubject['SubjectId']);
            let svgImage = svgRefs.current[intSubjectIndex];
            if (svgImage) {
                svgImage.setAttribute("height", intIntialHeight.toString() + "px");
    ...
    const GetSubjectProfile = (objTextResource, arrSubjectData) => {
        ...
        return (
            arrSubjectData.map(objSubjectData => {
                return (
                    <React.Fragment>
                        <div className="lernprofil-content-title">
                            {objSubjectData["SubjectName"]}
                        </div>
    ...
    */
    const GetSubjectProfileTemp = (objTextResource, arrSubjectData) => {
        ...
        svgRefs.current = arrSubjectData.map(
            (ref, index) => svgRefs.current[index] = React.createRef()
        )
        return (
            arrSubjectData.map((objSubject, intSubjectIndex) => {
                return (
                    <React.Fragment>
                        <div className="lernprofil-content-title">
                            {objSubject["SubjectName"]}
                        </div>
                        <div className="chart-image">
                            <div className="svg-baloon" id={"div_Result_Popup" + objSubject["SubjectId"]} hidden={!("div_Result_Popup" + objSubject.SubjectId === state.baloonSubjectId)}>
                                <div>{state.objResult.dtTestExecution}</div>
                                {state.objResult.Competency > 1 ?
                                    <React.Fragment>
                                        <div>{Localization.TextFormatter(objTextResource, 'CompetencyLevel')}{Localization.TextFormatter(objTextResource, state.objResult.Competency)} :{state.objResult.CompetencyLevel}</div>
                            ...
                            <div className="chart-image-flex" id={"div_ChartImage_" + objSubject['SubjectId']}>
                                <div className="svg-image">
                                    <table style={{
                                        width: "100%",
                                        height: objContext.LearnProfile_ModuleProcessor.GetTableHeight(objSubject) + 'px',
                                        background: "#efefef",
                                        ...
                                        <tbody>
                                            ...
                                            </tr>
                                            {
                                                objSubject['SubSubjects'].map(objOSubSubject => {
                                                    return (
                                                        <tr id={"tr_" + objOSubSubject['SubjectId']}>
                                                            <td>
                                                                <div className="text-block-flex">
                                                                    <div id={"txt_SubName_" + objOSubSubject['SubjectId']} className="block-left">
                                                                        <h3>{objOSubSubject['SubjectName']}</h3>
                                                                    </div>
                                                                    <div className="block-right">
                                                                        {
                                                                            objOSubSubject['Results'].length > 0 ?
                                                                                objOSubSubject['Results'].map((objResult, iResultIndex) => {
                                                                                    if (iResultIndex == 0) {
                                                                                        return (
                                                                                            <div><strong>{objResult['strTestName']}</strong></div>
                                                                                        );
                                                                                    } else {
                                                                                        return (
                                                                                            <div>{objResult['strTestName']}</div>
                                                                                        );
                                                                                    }
                                                                                }) : <React.Fragment />
                                                                        }
                                    ...
                                    {GetSvgChart(objSubject, Events, "div_Result_Popup" + objSubject["SubjectId"], intSubjectIndex)}
                                </div>
                            </div>
                        </div>
                    </React.Fragment>
    ...
    const GetSvgChart = (objSubject, Events, ParentSubjectId, intSubjectIndex) => {
        let svgRef = svgRefs.current[intSubjectIndex]
        let svg = <svg style={{
            width: "100%",
            height: objContext.LearnProfile_ModuleProcessor.GetTableHeight(objSubject) + 'px',
        ...
        >
            {
                objSubject['SubSubjects'].map((objOSubSubject, intSubSubjectIndex) => {
                    if (objOSubSubject['Lines']) {
                        lineRefs.current = objOSubSubject['Lines'].map(
                            (ref, index) => lineRefs.current[index] = React.createRef()
                        )
                    }
                        ...
                        <React.Fragment>
                            {
                                objOSubSubject['Lines'] ?
                                    objOSubSubject['Lines'].map((objLine, index) => {
                                        let objLineDim = objContext.LearnProfile_ModuleProcessor.GetLineDimensions(objLine, objOSubSubject['Results'])
                                        let lineRef = lineRefs.current[index]
                                        return (
                                            <line id={"line_" + objLine['uLineId']} ref={lineRef}
                                                style={{ fill: "none", stroke: "#FF4A75", "stroke-width": 3, "stroke-miterlimit": 10 }}
                                                //x1="67.5" y1="110" x2="67.5" y2="82"
                                                x1={objLineDim.x1} y1={objLineDim.y1} x2={objLineDim.x2} y2={objLineDim.y2}
                                            />
                                        );
                                    }) : <React.Fragment />
                            ...
                            }
                            {
                                objOSubSubject['Results'].length > 0 ?
                                    objOSubSubject['Results'].map((objResult, intResultIndex) => {
                                        let objCirDim = objContext.LearnProfile_ModuleProcessor.GetCircleRectDimensions(objContext, intSubSubjectIndex, objResult, intResultIndex);
                                        let objRectDim = objContext.LearnProfile_ModuleProcessor.GetCircleRectDimensions(objContext, intSubSubjectIndex, objResult, intResultIndex, true)
                                        return (
                                            <React.Fragment>
                                                <circle
                                                    id={"dotResult_" + objOSubSubject['SubjectId'] + '_' + objResult['uTestTokenId']}
                                                    style={{ fill: "#FF4A75", cursor: "pointer" }}
                                                    cx={objCirDim.cx} cy={objCirDim.cy} r={objCirDim.r}
                                                    onMouseOver={() => { Events.OnMouseOver("dotResult_" + objOSubSubject["SubjectId"] + '_' + objResult["uTestTokenId"], objResult, ParentSubjectId, objSubject.SubjectId); }}
                                                    onMouseLeave={() => { Events.OnMouseLeave(); }}
                                                />
                                                {
                                                    false ? //blnIsTabletView
                                                        <rect
                                                            id={"rctResult_" + objOSubSubject['SubjectId'] + '_' + objResult['uTestTokenId']}
                                                            x={objRectDim.x} y={objRectDim.y} width={objRectDim.width} height={objRectDim.height}
                                                            fill="transparent"
    ...
    */
    const GetContent = () => {
        let objTextResource = Object_Framework_Services_TextResource.GetData("/d.Extranet/4_Pupil/Modules/LearnProfile", objContext.props);
        let arrData = objContext.LearnProfile_ModuleProcessor.FormLines(objContext);
        return (
            <div className="lernprofil-content">
                <div className="lernprofil-header" id="LernProfilHeader" >
                    <PerformanceProfiler ComponentName={"LearnProfileWeekDisplay"} JConfiguration={props.JConfiguration} >
                        <WeekDisplay
                            Id={"LearnProfileWeekDisplay"}
                            JConfiguration={props.JConfiguration}
                            OnChangeDisplay={(objItem) => { objContext.LearnProfile_ModuleProcessor.OnChangeWeekDisplay(objContext, objItem); }}
                ...
                <WrapperComponent
                    ...
                    Id="LearnProfile_FillHeight"
                    Meta={{
                        HeaderIds: ["PupilHeader", "LernProfilHeader", "SubNavigationBlock"],
                        FooterIds: ["LernProfilFooter", "bottomSpacing"]
                    }}
                    ParentProps={{ ...props }}
                >
                ...
                <div className="lern-profil-footer" id="LernProfilFooter" />
            </div>
        );
    };
...
*/
export default connect(ExtranetBase_Hook.MapStoreToProps(LearnProfile_ModuleProcessor.StoreMapList()))(LearnProfile);

//<ChartOne objSubject={objSubject} Events={Events} ParentSubjectId={"div_Result_Popup" + objSubject["SubjectId"]} />

/**
 * @name ModuleProcessor
 * @summary Adding the Module_Processsor to export(for Prefetch)
 */
export const ModuleProcessor = LearnProfile_ModuleProcessor;