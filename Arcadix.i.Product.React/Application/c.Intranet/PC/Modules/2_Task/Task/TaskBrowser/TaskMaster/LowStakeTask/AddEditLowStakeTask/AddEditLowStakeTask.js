...
//Module related fies.
import * as AddEditLowStakeTask_Hook from '@shared/Application/c.Intranet/Modules/2_Task/Task/TaskBrowser/TaskMaster/LowStakeTask/AddEditLowStakeTask/AddEditLowStakeTask_Hook';
import AddEditLowStakeTask_ModuleProcessor from '@shared/Application/c.Intranet/Modules/2_Task/Task/TaskBrowser/TaskMaster/LowStakeTask/AddEditLowStakeTask/AddEditLowStakeTask_ModuleProcessor';
...
const AddEditLowStakeTask = props => {
     ...
     */
    const [state, dispatch] = useReducer(IntranetBase_Hook.Reducer, AddEditLowStakeTask_Hook.GetInitialState(props));
     ...
     */
    let objContext = { state, props, dispatch, "AddEditLowStakeTask_ModuleProcessor": new AddEditLowStakeTask_ModuleProcessor() };

    /**
     * @name Initialize
     * @param {object} objContext context object
     * @summary Initialize method call in AddEditLowStakeTask_Hook, that contains all the custom hooks.
     * @returns null
     */
    AddEditLowStakeTask_Hook.Initialize(objContext);
    ...
    const GetTaskDiv = () => {
        return <div id="TaskDiv" style={{
            ...
            <BasicProperties
                ...
                Events={{
                    OnKeyDown: (e) => objContext.AddEditLowStakeTask_ModuleProcessor.HandleEnterKeyDown(e, objContext.AddEditLowStakeTask_ModuleProcessor, objContext)
                }}
                ParentProps={{ ...props }}
            />
            ...
            <Taxonomy
                ...
                Data={{
                    DisplayData: state.objData,
                    SubjectId: props.Data.IsEdit ? objContext.AddEditLowStakeTask_ModuleProcessor.GetSubjectId(state.objData["iSubjectId"], objContext) : -1,
                    DropdownData: {
                ...
                Events={{
                    IsTheDropDownToShow: (strKey) => objContext.AddEditLowStakeTask_ModuleProcessor.IsTheDropDownToShow(strKey, objContext),
            ...
            <Language
                ...
                Events={{
                    IsShowLanguageDiv: () => objContext.AddEditLowStakeTask_ModuleProcessor.IsShowLanguageDiv(objContext)
                }}
                Resource={{
                    Text: props.Resource.Text,
    ...
    const GetDevelopmentHistoryDiv = () => {
        return <div style={{ display: (state.strDivToShow == "DevelopmentHistoryDiv" ? "block" : "none") }} className="task-tabcontent">
            ...
            <WorkflowStatus
                ...
                Events={{
                    GetAdministratorName: (strUserId) => objContext.AddEditLowStakeTask_ModuleProcessor.GetAdministratorName(strUserId, objContext)
                }}
                ParentProps={{ ...props }}
            />
        </div>
    ...
    */
    ...
    const GetAuditDiv = () => {
        return state.strDivToShow == "AuditDiv" ? <Audit
            ...
            CallBacks={{
                GetAuditUserName: (struUserId) => objContext.AddEditLowStakeTask_ModuleProcessor.GetAdministratorName(struUserId, objContext)
            }}
            ParentProps={{ ...props }}
        /> : <div />
    };
...
export default connect(IntranetBase_Hook.MapStoreToProps(AddEditLowStakeTask_ModuleProcessor.StoreMapList()))(AddEditLowStakeTask);