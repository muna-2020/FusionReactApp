...
const CMSTextHighlight_TestApplication = (props) => {
    ...
    */
    const GetElementJsonForComponent = (objContext, objLoadSolutionType, ElementJsonWithAnswer = null, UserAnswerJson = null, ElementEvaluationResult = null) => {
        ...
        let arrUserAnswerJson = [];
        if (ElementEvaluationResult) {
            arrUserAnswerJson = [...ElementEvaluationResult["UserAnswerJson"]];
        }
        arrNodes.forEach(objTempNode => {
            ...
            if (objLoadSolutionType["ViewSolution"]) {
                anchorElement.style.backgroundColor = ElementJsonWithAnswer.vElementJson.Values.find(e => e.iElementTextHighlightValueId.toString() === objTempNode.id.toString())["iElementTextHighlightValueColor"]
            }
            if (objLoadSolutionType["ViewComparison"]) {
                var objFilteredValues = arrUserAnswerJson.find(e => e.iElementTextHighlightValueId.toString() === objTempNode.id.toString());
                if (objFilteredValues) {
                    if (objFilteredValues["cIsCorrectValue"] == "Y") {//(actualSelectedColor === userSelectedColor) {
                        // correctly answered
                        anchorElement.classList.add('texthighlight-correct-answer');
                        anchorElement.style.backgroundColor = objFilteredValues["iElementTextHighlightValueColor"];
                    }
                    else {
                        // wrongly answered 
                ...
                //arrUserAnswerJson = arrUserAnswerJson.filter(e => e.iElementTextHighlightValueId.toString() !== objTempNode.id.toString())
            }
