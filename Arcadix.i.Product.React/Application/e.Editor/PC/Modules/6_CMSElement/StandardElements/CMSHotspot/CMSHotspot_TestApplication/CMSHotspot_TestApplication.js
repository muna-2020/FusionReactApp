...
const CMSHotspot_TestApplication = (props) => {
    ...
    useEffect(() => {
        if (objContext.state.ViewSolution || objContext.state.ViewComparison) {
            ...
            const { width, height } = objContext.imageArea.current.getBoundingClientRect();
            var canvas = objContext.myCanvas.current;
            canvas.width = width;
            canvas.height = height;
            var arrActualvalues = objContext.state.ElementJsonWithAnswer ? objContext.state.ElementJsonWithAnswer.vElementJson.Values : [];
            objContext.CMSHotspot_Common_ModuleProcessor.reDraw(objContext, arrActualvalues);
        }
    ...
    const GetContent = () => {
        ...
        if (objContext.state.ElementJson.vImageElementJson.vContainerElementProperties
            ...
            alignmentProperties = { "display": "flex", "justifyContent": vHorizontalAlignment, "alignItems": vVerticalAlignment, "height": "100%" };
        }
        return (
            <div ref={objContext.outerContainer} style={alignmentProperties} className="hotspot-test-application-main-div" ielementid={state.ElementJson["iElementId"]} ielementtypeid={state.ElementJson["iElementTypeId"]}>
                <div ref={objContext.innerContainer} style={{ "position": "relative" }} id="wrapper_div" onClick={(event) => { HandleImageMouseClick(event.clientX, event.clientY); }}>
                    <DragZone {...objDragZoneProps}>
                        <div DragDrop_DragAreaType="OptionArea">
                            {
                            ...
                            />
                            {
                                ...
                                <div type="load-solution-area" style={{ position: "absolute", "top": 0 }}>
                                    <canvas ref={objContext.myCanvas} id="myCanvas" style={{ "border": "1px dotted grey" }}></canvas>
                                    {
                                        objContext.state.ViewComparison && objContext.state.ElementEvaluationResult && objContext.state.ElementEvaluationResult.UserAnswerJson.map(v => {
                                            if (v.cIsCorrectValue == "N") {
                                                return (
                                                    <span className="wrong-answer-span"
                                                        type="wrong-answer-span"
                                                        style={{ zIndex: 0, position: "absolute", left: v.x - 25 + "px", top: v.y - 25 + "px" }}>
                                                    </span>
                                                );
                                            }
                                        })
