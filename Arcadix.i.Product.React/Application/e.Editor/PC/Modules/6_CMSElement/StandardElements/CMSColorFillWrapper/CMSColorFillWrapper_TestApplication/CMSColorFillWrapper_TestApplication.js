                ...
                <div id={`cms-color-fill-wrapper-${objContext.state.ElementJson["iElementId"]}`}
                    ...
                    }
                    {
                        ...
                        <div id={`color-fill-platte-${objContext.state.ElementJson.iElementId}`} className="color-fill-color-palette-wrapper">
                            {
                                objContext.state.ElementJson.vElementJson.vCapturedColors.map((objCaptuedValue, i) => {
                                    return (
                                        <div key={`outer-circle-${objContext.state.ElementJson["iElementId"]}-${i}`}
                                            className="color-fill-outer-circle" style={{ "backgroundColor": objCaptuedValue["vColorCode"], "border": objContext.state.strSelectedColor === objCaptuedValue["vColorCode"] ? "4px solid #000" : "none" }} onClick={() => { ChangeTextSelectionColor(objCaptuedValue["vColorCode"]); }}>
                                        </div>
                                    );
                                })
                            }
