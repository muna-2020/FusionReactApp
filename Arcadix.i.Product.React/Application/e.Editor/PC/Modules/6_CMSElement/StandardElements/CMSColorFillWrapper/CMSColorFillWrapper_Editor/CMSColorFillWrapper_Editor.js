    ...
    const SetAnswerRemoveRange = (objParams) => {
        ...
        if (element) {
            if (blnAddAnswer) {
                ...
                let strFillColor = element.getAttribute("fill");
                let objColorFillObject = objContext.arrColorFill.current.find(e => e.id == id);
                if (objColorFillObject && strFillColor) {
                    objColorFillObject["fill"] = strFillColor;
                    objContext.arrColorFill.current = [...objContext.arrColorFill.current.filter(e => e.id != id), objColorFillObject];
                }
                ...
                if (objClonedElementId) {
                    ...
                    let strFillColor = "";
                    strFillColor = clonedElement.getAttribute("fill");
                    clonedElement.remove();
                    objContext.arrCapturedElementIds.current = objContext.arrCapturedElementIds.current.filter(e => e.actualid !== id);
                    let objColorFillObject = objContext.arrColorFill.current.find(e => e.id == objClonedElementId.actualid);
                    if (objColorFillObject && strFillColor) {
                        objColorFillObject["fill"] = strFillColor;
                        objContext.arrColorFill.current = [...objContext.arrColorFill.current.filter(e => e.id != objClonedElementId.actualid), objColorFillObject];
                    }
                    objContext.dispatch({
                        "type": "SET_STATE", "payload": {
                            "ElementJson": {
        ...
        for (var intCount = 0; intCount < vColorFillJson.length; intCount++) {
            ...
            if (vColorFillJson[intCount].IsColorFill === "Y") {
                ...
                if (strColor) {
                    ...
                    targetClone.setAttribute("fill", strColor["vColorCode"]);
                    element.setAttribute("fill", strColor["vColorCode"]);
                }
                targetClone.style.stroke = "#0000ff";
                targetClone.style.strokeWidth = "8px";
                targetClone.style.strokeOpacity = "0.5";
