...
const Text_Editor = (props, ref) => {
            ...
            if (arrSubElements.length <= intIndex) {
                ...
                if (isHtmlLoaded) {
                    ...
                    if (objElementToWrap !== null) {
                        ...
                        let inumberofmediareplay = objElementToWrap.getAttribute("inumberofmediareplay");
                        if (objSubElementJson.vElementTypeName.toLowerCase() === "audio" && inumberofmediareplay != null && inumberofmediareplay != "") {
                            objSubElementJson = { ...objSubElementJson, ...CMSAudio_Editor_MetaData.GetDefaultContainerElementProperties(parseInt(inumberofmediareplay)) };
                        }
                    }
                }
    ...
    const ReplaceSubElementHtmlWithImg = (objElementJson, strInnerHtml) => {
        ...
        objElementJson.vElementJson.SubElements.forEach(objSubElement => {
            ...
            else if (objSubElement.vElementTypeName.toLowerCase() === 'audio') {
                let objDomAudio = document.createElement("img");
                let objAudioDiv = tempDiv.querySelector('[ielementid="' + objSubElement["iElementId"] + '"]');
                objDomAudio.id = objAudioDiv.id;
                objDomAudio.setAttribute("ielementid", objAudioDiv.id);
                objDomAudio.setAttribute("ielementaccessid", objAudioDiv.id);
                objDomAudio.setAttribute("ielementtypeid", objSubElement.iElementTypeId);
                objDomAudio.setAttribute("type", "Small_Audio");
                objDomAudio.setAttribute("inumberofmediareplay", objSubElement.vContainerElementProperties.iNumberOfMediaReplay);
                objAudioDiv.replaceWith(objDomAudio);
            }
        });
        /* develblock:start */
