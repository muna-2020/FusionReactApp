...
const Sidebar = (props) => {
    ...
    const ShowSidebar = (objSidebar) => {
        ...
        if (objEditorRef.current && objEditorRef.current !== null) {
            objEditorRef.current.SetSidebarStatus(objSidebar.SidebarProps.Position ? objSidebar.SidebarProps.Position : "right", true);
        }
    ...
    const HideSidebar = (fnCallback) => {
        ...
        let objEditorRef = EditorState.GetReference("EditorRef");
        if (objEditorRef.current && objEditorRef.current !== null) {
            objEditorRef.current.SetSidebarStatus("right", false);
        }
     ...
     */
    const GetContent = () => {
        let strClassName = "sidebar";
        if (props.Sidebar && props.Sidebar.SidebarProps && props.Sidebar.SidebarProps.Position) {
            strClassName = props.Sidebar.SidebarProps.Position.toLowerCase() === "left" ? "sidebar left-sidebar" : "sidebar right-sidebar";
        }

        return (
            props.Sidebar ?
                <div className={strClassName} style={{ display: props.Sidebar && props.Sidebar.SidebarProps && props.Sidebar.SidebarProps.Status === 1 ? "block" : "none" }}>
