...
const CMSImage_TestApplication = (props, ref) => {
  ...
  const [state, dispatch] = useReducer(
    Base_Hook.Reducer,
    CMSImage_TestApplication_Hook.GetInitialState(props)
  );
  ...
  let objContext = {
    ...
    props,
    dispatch,
    ModuleName: "CMSImage_TestApplication_" + props.ElementJson.iElementId,
  ...
  const GetContent = () => {
    ...
    if (
      ...
      let objElementHeader = objContext.state.ElementJson["vElementJson"][
        "HeaderValues"
      ].filter(
        (objTempHeaderValue) =>
          objTempHeaderValue["vHeaderType"] === "ElementHeader"
      )[0];
      objTextElementProps = objContext.CMSImage_TestApplication_ModuleProcessor.GetTextElementProps(
        objContext,
        objElementHeader["iElementTextId"]
      );
    }
    ...
    if (
      ...
      objContext.state.ElementJson.vContainerElementProperties
        .vElementVerticalAlignment &&
      objContext.state.ElementJson.vContainerElementProperties
        .vElementHorizontalAlignment
    ) {
      var vHorizontalAlignment = objContext.CMSImage_TestApplication_ModuleProcessor.GetAlignmentValue(
        "horizontal",
        objContext.state.ElementJson.vContainerElementProperties
          .vElementHorizontalAlignment
      );
      var vVerticalAlignment = objContext.CMSImage_TestApplication_ModuleProcessor.GetAlignmentValue(
        "vertical",
        objContext.state.ElementJson.vContainerElementProperties
          .vElementVerticalAlignment
      );
      alignmentProperties = {
        display: "flex",
        justifyContent: vHorizontalAlignment,
        alignItems: vVerticalAlignment,
      };
    }
    return (
      <div
        style={alignmentProperties}
        ielementid={objContext.state.ElementJson.iElementId}
        ielementtypeid={objContext.state.ElementJson.iElementTypeId}
      >
        {objContext.state.ElementJson["vElementJson"]["cShowHeaderText"] ===
        "Y" ? (
        ...
        <div id="image_wrapper">
          <img
            ...
            alt={
              objContext.state.ElementJson["vElementJson"]["vElementImageName"]
            }
          />
        </div>
      </div>
 ...
 */
export const ModuleProcessor = CMSImage_TestApplication_ModuleProcessor; 