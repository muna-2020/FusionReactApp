...
/**
 * @name UpdateUndoRedoIconState
 * @summary update the undo-redo icon status to TopLeft component.
 */
export const UpdateUndoRedoIconState = () =>{ 
    let objGlobalState = EditorState.GetProperty("UndoRedoQueue"); 
    let objEditorRef = EditorState.GetReference("EditorRef"); 
    if(objEditorRef && objEditorRef.current && objEditorRef.current !== null){ 
        let intActiveTaskId = objEditorRef.current.GetActiveTaskId();
        let objTopLeftRef = EditorState.GetReference("TopLeftRef");
        if(objGlobalState[intActiveTaskId] && objEditorRef && objEditorRef.current !== null && objTopLeftRef && objTopLeftRef.current !== null){
            let strUndoState = objTopLeftRef.current.GetState().UndoState;
            let strRedoState = objTopLeftRef.current.GetState().RedoState;
            if(objGlobalState[intActiveTaskId].ActivityQueue.length > 1){
                strUndoState = "active";
            }else {
                strUndoState = "inactive";
            }
            if(objGlobalState[intActiveTaskId].RedoQueue.length > 0){
                strRedoState = "active"
            }else {
                strRedoState = "inactive";
            }    
            console.log("UndoRedo icon state", {"UndoState" : strUndoState, "RedoState" : strRedoState});
            objTopLeftRef.current.SetUndoRedoIconStatus({"UndoState" : strUndoState, "RedoState" : strRedoState});
        }
    }
}
