...
/**
 * @name AddEditClientHostUrl_ModuleProcessor
 * @summary Class for Add/Edit ClientHostUrl module.
 */
class AddEditClientHostUrl_ModuleProcessor extends CockpitBase_ModuleProcessor{
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iHostURLId"] && objContext.state.objData["iHostURLId"] != "") {
                objContext.props.Data.Object_Cockpit_ClientHostUrl.EditData({ "vEditData": [objContext.state.objData] }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "ClientHostUrlGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                objContext.props.Data.Object_Cockpit_ClientHostUrl.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    this.ResetGridSelection("ClientHostUrlGrid");    
                    if (blnClose) {
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
    ...
    * @summary   To change the row Data on change of the dropdown value
    */
    HandleApplicationtypeDropDownChange(strAttributeName, objChangeData, props, objContext) {
        let strCellName = props.Meta["ValueColumn"];
        let strValue = objChangeData[strCellName];
        let objNewData = CockpitBase_Form.HandleChange(objContext, strAttributeName, strValue, "objData");
        objNewData = { ...objNewData,"iClientId":-1}
        objContext.dispatch({ type: "SET_STATE", payload: { "objData": objNewData } });
    }
...
export default AddEditClientHostUrl_ModuleProcessor;