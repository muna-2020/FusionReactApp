...
/**
 * @name AddEditClient_ModuleProcessor
 * @summary Class for Add/Edit Client module.
 */
class AddEditClient_ModuleProcessor extends CockpitBase_ModuleProcessor{
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iClientId"] && objContext.state.objData["iClientId"] != "") {
                objContext.props.Data.Object_Cockpit_Client.EditData({ "vEditData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "ClientGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                objContext.props.Data.Object_Cockpit_Client.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
...
export default AddEditClient_ModuleProcessor;