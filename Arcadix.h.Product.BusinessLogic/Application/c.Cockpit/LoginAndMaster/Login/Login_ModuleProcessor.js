...
import Cokpit_LoginAndMaster_Login from '@shared/Application/c.Cockpit/LoginAndMaster/Login/Login_Module'
...
class Login_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    LoadInitialData(objContext) {
        //(new ObjectQueue()).QueueAndExecute(this.InitialDataParams(objContext.props), null, this, objContext.props);
    ...
    //ValidateOnBlur(objContext) {
    //    const arrMeta = this.GetMetaData();
    //    let objNewValidationMessageJson = ValidateClientSide(arrMeta, JSON.parse(objContext.props.JConfiguration["LoginTextResources"]), objContext.state.objData, "", false, "divValidationMessage");
    ...
    */
    OnLoginClick(objContext) {
        ...
        if (objNewValidationMessageJson == null) {
            ...
            Cokpit_LoginAndMaster_Login.ValidateUser(objParams, (objResponse) => {
                if (objResponse.login.Data.Success == true) {
                    ...
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "strBarcodeImageUrl": objResponse.login.Data.BarcodeImageUrl } });
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "txtUsername": objResponse.login.Data.UserId } });
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "strUserValidated": true } });
    ...
    */
    OnTwoFactorAuthenticateClick(objContext) {
        if (objContext.state.objData.strAuthenticationCode != "") {
            var objParams = {
                UserId: objContext.state.txtPassword,
                AuthenticationCode: objContext.state.objData.strAuthenticationCode,
            }
            Cokpit_LoginAndMaster_Login.TwoFactorAuthenticate(objParams, (objResponse) => {
