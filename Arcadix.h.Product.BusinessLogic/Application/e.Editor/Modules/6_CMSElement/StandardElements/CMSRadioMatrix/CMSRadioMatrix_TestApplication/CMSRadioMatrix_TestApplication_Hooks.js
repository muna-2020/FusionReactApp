...
function useDataLoaded(objContext) {
    useEffect(() => {
        ...
        let objLoadSolutionType = objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.GetLoadSolutionType(objContext, objElementJsonWithAnswer, objUserAnswerJson);
        let objElementJson = objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.GetElementJsonForComponent(objContext, objLoadSolutionType, objElementJsonWithAnswer, objUserAnswerJson);
        objContext.dispatch({
            type: "SET_STATE",
            payload: {
                "ElementJson": {
...
function useImperativeMethods(objContext) {
    useImperativeHandle(objContext.props.ElementRef, () => ({
        ...
        "LoadSolution": (ElementJsonWithAnswer = null, UserAnswerJson = null, ElementEvaluationResult = null) => {
            objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.LoadSolution(objContext, ElementJsonWithAnswer, UserAnswerJson, ElementEvaluationResult);
            let objLoadSolutionType = objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.GetLoadSolutionType(objContext, ElementJsonWithAnswer, UserAnswerJson);
            let objElementJson = objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.GetElementJsonForComponent(objContext, objLoadSolutionType, ElementJsonWithAnswer, UserAnswerJson);
            objContext.dispatch({
                "type": "SET_STATE",
                "payload": {
                    ...objLoadSolutionType,
                    "ElementJson": objElementJson,
                    "arrRadioMatrixAnswered": objElementJson["vElementJson"]["Values"].map(objTempData => {
                        return {
                            "iElementRadioMatrixRowId": objTempData["iElementRadioMatrixRowId"],
                            "iElementRadioMatrixColumnId": objTempData["iElementRadioMatrixColumnId"]
                        };
                    }),
        ...
        "ResetAnswer": () => {
            objContext.CMSRadioMatrix_TestApplication_ModuleProcessor.ResetAnswer(objContext);
        }
