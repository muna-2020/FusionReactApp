...
function useDataLoaded(objContext) {
    useEffect(() => {
        ...
        let objLoadSolutionType = objContext.CMSDragdropSort_TestApplication_ModuleProcessor.GetLoadSolutionType(objContext, objElementJsonWithAnswer, objUserAnswerJson);
        let objElementJson = objContext.CMSDragdropSort_TestApplication_ModuleProcessor.GetElementJsonForComponent(objContext, objLoadSolutionType, objElementJsonWithAnswer, objUserAnswerJson);
        objContext.dispatch({
            "type": "SET_STATE",
            "payload": {
                "ElementJson": {
    ...
    useImperativeHandle(objContext.props.ElementRef, () => ({
        "GetUserResponse": () => {
            ...
            let arrValues = objContext.state.ElementJson["vElementJson"]["Values"].map(objTempValue => {
                let { iTempDisplayOrder, ...objValue } = objTempValue;
                return objValue;
            });
            arrResponse = [
                {
                    ...
                    ["cIsAnswered"]: objContext.state.ElementJson["vElementJson"]["Values"].filter(objTempData => objTempData["iDisplayOrder"] !== -1).length > 0 ? "Y" : "N"
                }
            ];
            return arrResponse;
        },
        "LoadSolution": (ElementJsonWithAnswer = null, UserAnswerJson = null, ElementEvaluationResult = null) => {
            objContext.CMSDragdropSort_TestApplication_ModuleProcessor.LoadSolution(objContext, ElementJsonWithAnswer, UserAnswerJson, ElementEvaluationResult);
            let objLoadSolutionType = objContext.CMSDragdropSort_TestApplication_ModuleProcessor.GetLoadSolutionType(objContext, ElementJsonWithAnswer, UserAnswerJson);
            let objElementJson = objContext.CMSDragdropSort_TestApplication_ModuleProcessor.GetElementJsonForComponent(objContext, objLoadSolutionType, ElementJsonWithAnswer, UserAnswerJson);
            objContext.dispatch({
                "type": "SET_STATE",
                "payload": {
                    ...objLoadSolutionType,
                    "ElementJson": objElementJson,
                    "arrDragdropSortAnswered": [...objElementJson["vElementJson"]["Values"]],
        ...
        "ResetAnswer": () => {
            objContext.CMSDragdropSort_TestApplication_ModuleProcessor.ResetAnswer(objContext);
        }
