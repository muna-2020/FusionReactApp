...
function useImperativeMethods(objContext) {
    ...
    useImperativeHandle(objContext.props.ElementRef, () => ({
        "GetElementJson": async (blnRemoveRef = true, strDataFor = null) => {
            ...
            let objElementJson = {
                ...
                ["vElementJson"]: {
                    ...
                    // "Coordinates": objContext.Coordinates_Ref.current,
        ...
        "SpellCheckUpdate": (objCheckedElementJson) => { //optional needed if element has Text_Editor.
            objContext.CMSMapElement_Editor_ModuleProcessor.ElementSpellCheckUpdate(objContext, objCheckedElementJson);
        },
        "GetContextMenuOptions": (intElementTextId) => {
            ...
            return objContext.CMSMapElement_Editor_ModuleProcessor.GetContextMenuOptions(objContext, { "Value": arrData[0], "Type": strType });
        },
        "PreserveTextState": (objState) => {
            objContext.CMSMapElement_Editor_ModuleProcessor.PreserveTextState(objContext, objState);
        },
        "GetPointOverride": () => {
            return objContext.CMSMapElement_Editor_ModuleProcessor.GetPointOverride(objContext)
        },
        "SetPointOverride": (objPoints) => {
            objContext.CMSMapElement_Editor_ModuleProcessor.SetPointOverride(objContext, objPoints);
        },
        "RemovePointOverride": (objPoints) => {
            objContext.CMSMapElement_Editor_ModuleProcessor.RemovePointOverride(objContext);
        },
