...
/**
 * @name AddEditSubject_ModuleProcessor
 * @param NA
 * @summary Class for Add/Edit Language module.
 */
class AddEditSubject_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iSubjectId"] && objContext.state.objData["iSubjectId"] != "") {
                Object_Intranet_Taxonomy_Subject.EditData({ "vEditData": [objContext.state.objData] }, (objReturn, blnEdited) => {
                    if (blnEdited) {
                        objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "SubjectGrid": [objReturn[0]] });
                        if (blnClose)
                ...
                Object_Intranet_Taxonomy_Subject.AddData({ "vAddData": objContext.state.objData }, (objReturn, blnAdded) => {
                    if (blnAdded) {
                        objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
...
export default AddEditSubject_ModuleProcessor;