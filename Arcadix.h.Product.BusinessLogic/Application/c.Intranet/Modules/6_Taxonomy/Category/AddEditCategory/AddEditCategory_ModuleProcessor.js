...
/**
 * @name AddEditCategory_ModuleProcessor
 * @summary Class for Add/Edit Category module.
 */
class AddEditCategory_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iCategoryId"] && objContext.state.objData["iCategoryId"] != "") {
                ApplicationState.SetProperty("blnShowAnimation", true);
                Object_Intranet_Taxonomy_Category.EditData({ "vEditData": [objContext.state.objData] }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    ApplicationState.SetProperty("blnShowAnimation", false);
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "CategoryGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                Object_Intranet_Taxonomy_Category.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
...
export default AddEditCategory_ModuleProcessor;