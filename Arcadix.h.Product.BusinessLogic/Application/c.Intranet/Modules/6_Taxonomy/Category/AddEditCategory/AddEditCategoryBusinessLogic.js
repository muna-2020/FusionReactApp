...
export function useInitializeRibbon(objContext) {
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupTabData", { "addeditcategorypopup": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addeditcategorypopup": arrRibbonData });
    }, [objContext.state]);
}
...
export function useInitializeTabs(objContext) {
    let arrContentData = [
        {//Group1
            "Text": objContext.props.Data.objTextResource["CategoryManagement"],
            "Id": "NavId1",
            "Children": [
                {
                    "Text": objContext.props.Data.objTextResource["CategoryManagement"],
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupNavigationData", { "addeditcategorypopup": arrContentData });
    },
        []);
}
...
export function useInitializeData(objContext) {
    ...
    var objAddData = {
        "iCategoryId": null,
        "iSubjectId": objContext.props.Data.intSubSubjectDropdownSelectedValue,
        "iMainClientId": objContext.props.Data.JConfiguration.MainClientId, //97,
        "iDisplayOrder": null,
        "uUserId": objContext.props.Data.ClientUserDetails.UserId,
        "dtCreatedOn": null,
        "vCategoryKeyword": null,
        "cIsDeleted": "N",
        "t_TestDrive_Category_Data": GetMultiLanguageData(objContext)["arrMultiLanguageData"]
    };
    ...
    if (objContext.props.Data.MainClientLanguageData && objContext.props.Data.LanguageData) {
        ...
        objContext.props.Data.LanguageData.map((objLanguage) => {
            if (arrClientLanguageId.includes(objLanguage["iFrameworkLanguageId"])) {
                let obj = {
                    "iCategoryId": null,
                    "iLanguageId": objLanguage["iFrameworkLanguageId"], //props.Data.foreignKey,
                    "vCategoryName": "",
                    "tConceptAndSkills": "",
                    "tClarificationNotes": ""
...
export function ModifyEditData(objContext, arrSelectedRows, objTempEditData) {
    ...
    if (objContext.props.Data.blnIsEdit && arrSelectedRows) {
        ...
        arrMultiLanguageData.map((objMultiLanguageData) => {
            let arrTempSubjectData = objTempEditData["t_TestDrive_Category_Data"].filter((objTableData) => objTableData["iLanguageId"].toString() === objMultiLanguageData["iLanguageId"].toString());
        ...
        objNewEditData = { ...objTempEditData, "t_TestDrive_Category_Data": arrSubjectData };
    }
...
export const AddMethod = (objContext, blnClose = false) => {
    ...
    let arrMetaData = [GetObjectHeader("t_TestDrive_Category_Data.vCategoryName", objContext), GetObjectHeader("iDisplayOrder", objContext)];
    ...
    if (blnIsValid) {
        ...
        if (objContext.props.Data.blnIsAdd && !objContext.state.blnAddedOnce) {
            let objCategoryParams = {
                "SearchQuery": {
                    "must": [
                        {
                            "match": {
                                "iSubjectId": objContext.props.Data.intSubSubjectDropdownSelectedValue.toString()
                            }
                        }
                    ]
                },
            ...
            objArcadixFetchAndCacheData.Execute(arrParams, function (objReturn, cIsNewData) {
                if (cIsNewData) {
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "objCategory": objReturn[0] } });
            ...
            if (objContext.state.blnAddedOnce) {
                ...
                objEditData["iCategoryId"] = objContext.state.objCategory["iCategoryId"];
            }
            let objCategoryParams = {
                "SearchQuery": {
                    "must": [
