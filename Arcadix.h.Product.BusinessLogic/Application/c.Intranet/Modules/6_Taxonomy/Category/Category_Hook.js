...
export function useDataLoader(objContext) {
    useLayoutEffect(() => {
        objContext.Category_ModuleProcessor.LoadInitialData(objContext);
    }, []);
}
...
export function useDataLoaded(objContext) {
    useEffect(() => {
        ...
        if (!objContext.state.isLoadComplete
            ...
            && Object_Framework_Services_TextResource.GetData("/c.Intranet/Modules/6_Taxonomy/Category", objContext.props)
...
export function useSetRibbonData(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            objContext.Category_ModuleProcessor.SetRibbonData(objContext);
        }
    }, [objContext.state]);
