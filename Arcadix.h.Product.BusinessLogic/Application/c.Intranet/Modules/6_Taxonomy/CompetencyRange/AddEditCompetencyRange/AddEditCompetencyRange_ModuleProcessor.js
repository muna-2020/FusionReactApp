...
/**
 * @name AddEditCompetencyRange_ModuleProcessor
 * @summary Class for Add/Edit CompetencyRange module.
 */
class AddEditCompetencyRange_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iCompetencyRangeId"] && objContext.state.objData["iCompetencyRangeId"] != "") {
                ApplicationState.SetProperty("blnShowAnimation", true);
                Object_Intranet_Taxonomy_CompetencyRange.EditData({ "vEditData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    ApplicationState.SetProperty("blnShowAnimation", false);
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "CompetencyRangeGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });

            } else {
                Object_Intranet_Taxonomy_CompetencyRange.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
...
export default AddEditCompetencyRange_ModuleProcessor;