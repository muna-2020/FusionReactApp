...
export function GetInitialState(props) {
    ...
    if (
        ...
        && Object_Framework_Services_TextResource.GetData("/c.Intranet/Modules/6_Taxonomy/CompetencyRange", props)
...
export function useDataLoader(objContext) {
    useLayoutEffect(() => {
        objContext.CompetencyRange_ModuleProcessor.LoadInitialData(objContext);
    }, []);
}
...
export function useDataLoaded(objContext) {
    useEffect(() => {
        ...
        if (!objContext.state.isLoadComplete
            ...
            && Object_Framework_Services_TextResource.GetData("/c.Intranet/Modules/6_Taxonomy/CompetencyRange", objContext.props)
...
export function useSetRibbonData(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            objContext.CompetencyRange_ModuleProcessor.SetRibbonData(objContext);
        }
    }, [objContext.state]);
