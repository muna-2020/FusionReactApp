...
export function mapStateToProps(state) {
    if (!global["mode"]) {
        return {
            category: DataRef(state.Entity, "object_intranet_taxonomy_categorycompetency", true)
        };
    }
    else {
...
export function useInitializeRibbon(objContext) {
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupTabData", { "addeditcategorycompetency": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addeditcategorycompetency": arrRibbonData });
    }, [objContext.state]);
}
...
export function useInitializeTabs(objContext) {
    let arrContentData = [
        {//Group1
            "Text": objContext.props.Data.objTextResource["CategoryCompetency"],
            "Id": "NavId1",
            "Children": [
                {
                    "Text": objContext.props.Data.objTextResource["CategoryCompetency"],
                    ...
                    "Event": () => {
                        //ShowDiv("CategoryManagement", objContext)
                    }
                }
            ]
        }
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupNavigationData", { "addeditcategorycompetency": arrContentData });
    },
        [])
}
...
export function useInitializeData(objContext) {
    ...
    var objValidationColumnTabMapping = {
        "iDisplayOrder": "CategoryCompetency",
    }

    var objAddData = {
        "iCategoryCompetencyId": null,
        "iCategoryId": objContext.props.Data.intCategoryDropdownSelectedValue,
        "iMainClientId": objContext.props.Data.JConfiguration.MainClientId,
        "iDisplayOrder": null,
        "cIsDeleted": "N",
        "vCompetencykeyword": null,
        "t_TestDrive_Category_Competency_Data": GetMultiLanguageData(objContext)["arrMultiLanguageData"]
    };
...
export function ModifyEditData(objContext, arrSelectedRows, objTempEditData) {
    ...
    if (objContext.props.Data.blnIsEdit && arrSelectedRows) {
        ...
        arrMultiLanguageData.map((objMultiLanguageData) => {
            let arrTempSubjectData = objTempEditData["t_TestDrive_Category_Competency_Data"].filter((objTableData) => objTableData["iLanguageId"].toString() === objMultiLanguageData["iLanguageId"].toString())
        ...
        objNewEditData = { ...objTempEditData, "t_TestDrive_Category_Competency_Data": arrSubjectData }
    }
    ...
    if (blnIsValid) {
        ...
        if (objContext.props.Data.blnIsAdd && !objContext.state.blnAddedOnce) {
            let objCategoryCompetencyParams = {
                "SearchQuery": {
                    "must": [
                        {
                            "match": {
                                "iCategoryId": objContext.props.Data.intCategoryDropdownSelectedValue.toString()
                            }
                        },
                    ]
                },
            ...
            objArcadixFetchAndCacheData.Execute(arrParams, function (objReturn, cIsNewData) {
                if (cIsNewData) {
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "objCategoryCompetency": objReturn[0] } });
            ...
            if (objContext.state.blnAddedOnce) {
                ...
                objEditData["iCategoryCompetencyId"] = objContext.state.objCategoryCompetency["iCategoryCompetencyId"];
            }
            let objCategoryCompetencyParams = {
                "SearchQuery": {
                    "must": [
...
export function GetInitialState() {
    return {
        ...
        strDivToShow: "CategoryManagement",
    };
}
