...
/**
 * @name AddEditCategoryCompetency_ModuleProcessor
 * @summary Class for Add/Edit CategoryCompetency module.
 */
class AddEditCategoryCompetency_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["iCategoryCompetencyId"] && objContext.state.objData["iCategoryCompetencyId"] != "") {
                ApplicationState.SetProperty("blnShowAnimation", true);
                Object_Intranet_Taxonomy_CategoryCompetency.EditData({ "vEditData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    ApplicationState.SetProperty("blnShowAnimation", false);
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "CategoryCompetencyGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                Object_Intranet_Taxonomy_CategoryCompetency.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
...
export default AddEditCategoryCompetency_ModuleProcessor;