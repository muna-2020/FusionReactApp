...
import ApplicationState from '@shared/Framework_Old/DataService/ApplicationState/ApplicationState';
import * as FieldValidator from "@root/Framework_Old/Services/Validator/Validator";
...
function GetInitilaData(objContext){
    let objAddData = {
        ...
        "iTestParentFolderID":objContext.props.FolderId,
...
export function SetApplicationState(objContext) {
    useEffect(()=>{
        ...
        let Navdata = [
            {//Group1
                "Text": objContext.props.Data.objTextResource["Create"],
                "Id": "NavId1",
                "Children": [
                    {
        ...
        ApplicationState.SetProperty("PopupTabData", { "addedittestfolder": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addedittestfolder": RibbonData });
        ApplicationState.SetProperty("PopupNavigationData", { "addedittestfolder": Navdata });
    },[objContext.state])    
}
...
export function HandleCheckBoxChange(strId, objContext){
    var objNewData = {...objContext.state.objData,[strId]:objContext.state.objData[strId]=="Y"?"N":"Y"};
    objContext.dispatch({ type: "SET_STATE_VALUES", payload: {objData : objNewData} });
}
...
export function AddMethod(objContext){
    objContext.dispatch({ type: "SET_STATE_VALUES", payload: {blnSaveClicked : true} });
    let arrMetaData = GetRespectiveHeaders(["vTestFolderName"],objContext.props);
