...
export function mapStateToProps(state) {
    if (!global["mode"]) {
        return {
            schoolyear: DataRef(state.Entity, "object_extranet_teacher_schoolyear", true)
        };
    }
    else {
...
export function useInitializeRibbon(objContext) {
    ...
    useEffect(() => {
        ...
        ApplicationState.SetProperty("PopupTabData", { "addeditschoolyear": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addeditschoolyear": arrRibbonData });
    }, [objContext.state]);
}
...
export function useInitializeTabs(objContext) {
    let arrContentData = [
        {//Group1
            "Text": objContext.props.Data.objTextResource["SchoolYearManagement"],
            "Id": "NavId1",
            "Children": [
                {
                    "Text": objContext.props.Data.objTextResource["SchoolYearManagement"],
                    "Id": "CompetencyLevel",
                    "Event": () => {
                        ShowDiv("SchoolYearManagement", objContext)
                    }
                }
            ]
        }
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupNavigationData", { "addeditschoolyear": arrContentData });
    },
        [])
}
...
export function useInitializeData(objContext) {
    ...
    var objValidationColumnTabMapping = {
        "iSchoolYear": "SchoolYearManagement",
        "iDisplayOrder": "SchoolYearManagement",
    };
    ...
    var objAddData = {
        ...
        "iDisplayOrder": null,
        "cIsDeleted": "N",
        "t_TestDrive_Member_Class_SchoolYear_Data": GetMultiLanguageData(objContext)["arrMultiLanguageData"]
    };
...
export function ModifyEditData(objContext, arrSelectedRows, objTempEditData) {
    ...
    if (objContext.props.Data.blnIsEdit && arrSelectedRows) {
        ...
        arrMultiLanguageData.map((objMultiLanguageData) => {
            let arrTempSubjectData = objTempEditData["t_TestDrive_Member_Class_SchoolYear_Data"].filter((objTableData) => objTableData["iLanguageId"].toString() === objMultiLanguageData["iLanguageId"].toString());
        ...
        objNewEditData = { ...objTempEditData, "t_TestDrive_Member_Class_SchoolYear_Data": arrSubjectData };
    }
...
export const AddMethod = (objContext, blnClose = false) => {
    ...
    let arrMetaData = [GetObjectHeader("iSchoolYear", objContext), GetObjectHeader("iDisplayOrder", objContext)];
    var blnIsValid = true;
    ...
    if (blnIsValid) {
        ...
        if (objContext.props.Data.blnIsAdd && !objContext.state.blnAddedOnce) {
            ...
            objArcadixFetchAndCacheData.Execute(arrParams, function (objReturn, cIsNewData) {
                if (cIsNewData) {
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "objCompetencyRange": objReturn[0] } });
            ...
            if (objContext.state.blnAddedOnce) {
                ...
                objEditData["iSchoolYearId"] = objContext.state.objCompetencyRange["iSchoolYearId"];
            }
            let objSchoolYearParams = {
