...
import * as AddEditState_MetaData from '@shared/Application/c.Intranet/Modules/8_Setting/State/AddEditState/AddEditState_MetaData';

/**
 * @name AddEditState_ModuleProcessor
 * @summary Class for Add/Edit State module.
 */
class AddEditState_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            ...
            if (objContext.state.objData["iStateId"] && objContext.state.objData["iStateId"] != "") {                
                Object_Extranet_State_State.EditData({ "vEditData": [objContext.state.objData], "uUserId": objContext.props.ParentProps.ClientUserDetails.UserId }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    ApplicationState.SetProperty("blnShowAnimation", false);
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "StateGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                Object_Extranet_State_State.AddData({ "vAddData": objContext.state.objData, "uUserId": objContext.props.ParentProps.ClientUserDetails.UserId }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
    ...
    Validate(objContext, strColumnName) {
        var objNewValidationObject = FieldValidator.ValidateClientSide(AddEditState_MetaData.GetAddEditMetaData(), objContext.props.Resource.Text, objContext.state.objData, strColumnName, true, "ValidationError");
    ...
    ValidateOnBlur(strColumnName, objContext) {
        if (objContext.state.blnSaveClicked) {
            FieldValidator.ValidateClientSide(AddEditState_MetaData.GetAddEditMetaData(), objContext.props.Resource.Text, objContext.state.objData, strColumnName, false, "", "", true)
        }
    }
}
export default AddEditState_ModuleProcessor;