...
export function useInitializeRibbon(objContext) {
    ...
    useEffect(() => {
        ...
        ApplicationState.SetProperty("PopupTabData", { "addeditroles": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addeditroles": arrRibbonData });
    }, [objContext.state]);
}
...
export function useInitializeTabs(objContext) {
    let arrContentData = [
        {//Group1
            "Text": objContext.props.Data.objTextResource["UserRoleName"],
            "Id": "NavId1",
            "Children": [
                {
                    "Text": objContext.props.Data.objTextResource["UserRoleName"],
                    "Id": "UserRoleName",
                    "Event": () => {
                        ShowDiv("UserRoleName", objContext)
                    }
                }
            ]
        }
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupNavigationData", { "addeditroles": arrContentData });
    },
        [])
}
...
export function useInitializeData(objContext) {
    ...
    var objValidationColumnTabMapping = {
        "t_Framework_MainClient_UserRole_Data.vUserRoleName": "UserRoleName",
    };

    var objAddData = {
        "cIsDeleted": "N",
        "cIsSystemRole": "N",
        "cIsWorkflowRole": null,
        "iMainClientId": objContext.props.Data.ClientUserDetails["MainClientId"],
        "iApplicationTypeId": objContext.props.Data.ClientUserDetails["ApplicationTypeId"],
        "uUserId": objContext.props.Data.ClientUserDetails["UserId"],
        "t_Framework_MainClient_UserRole_Data": GetMultiLanguageData(objContext)["arrMultiLanguageData"]
    }
...
export function ModifyEditData(objContext, arrSelectedRows, objTempEditData) {
    ...
    if (objContext.props.Data.blnIsEdit && arrSelectedRows) {
        ...
        arrMultiLanguageData.map((objMultiLanguageData) => {
            let arrTempSubjectData = objTempEditData["t_Framework_MainClient_UserRole_Data"].filter((objTableData) => objTableData["iLanguageId"].toString() === objMultiLanguageData["iLanguageId"].toString());
        ...
        objNewEditData = { ...objTempEditData, "t_Framework_MainClient_UserRole_Data": arrSubjectData };
    }
...
export const AddMethod = (objContext, blnClose = false) => {
    ...
    let arrMetaData = [GetObjectHeader("t_Framework_MainClient_UserRole_Data.vUserRoleName", objContext)];
    var blnIsValid = true;
    ...
    if (blnIsValid) {
        ...
        if (objContext.props.Data.blnIsAdd && !objContext.state.blnAddedOnce) {
            ...
            if (objContext.state.blnAddedOnce) {
                ...
                objEditData["uUserRoleId"] = objContext.state.objBusinessUnit["uUserRoleId"];
            }
            arrParams = [
                {
                    "URL": "API/Object/Cockpit/UserRole",
