    ...
    let objResourceParams = {
        ...
        "SearchQuery": {
            "must": [
                {
                    "match": {
                        "Id": JConfiguration.LanguageCultureInfo + "/c.Intranet/Modules/8_Setting/BusinessUnitTeamRole/BusinessUnit"
                    }
                }
            ]
        }
...
export function useDataLoaded(objContext) {
    useEffect(() => {
        if (!objContext.state.isLoadComplete
            ...
            && DataRef(objContext.props.textresource, "textresource;id;" + objContext.props.JConfiguration.LanguageCultureInfo + "/c.intranet/modules/8_setting/businessunitteamrole/businessunit")
            ...
            objContext.dispatch({
                type: "SET_STATE_VALUES", payload: {
                    ...
                    "objTextResource": DataRef(objContext.props.textresource, "textresource;id;" + objContext.props.JConfiguration.LanguageCultureInfo + "/c.intranet/modules/8_setting/businessunitteamrole/businessunit").Data["0"].BusinessUnit
                }
            });
...
function AddPopup(objContext) {
    ...
    objContext.props.showPopup({
        ...
        Data: {
            ModuleName: "addeditbusinessunit",
            arrHeaderData,
            objTextResource,
    ...
    if (arrSelectedRows && arrSelectedRows.length > 0) {
        objContext.props.showPopup({
            ...
            Data: {
                ...
                OnConfirmationClick: (objModal) => DeleteBusinessUnit(arrSelectedRows, objModal, objContext)
            }
        });
    } else {
        ErrorPopup(objContext);
...
*/
const DeleteBusinessUnit = (arrSelectedRows, objModal, objContext) => {
    let arrDeleteRow = [];
    ...
    return [
        {
            ...
            "vDataType": "string",
            "iDisplayOrder": 1,
            "vTextResourcePage": "BusinessUnit",
...
*/
export function useOnBusinessUnitUpdate(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "arrBusinessUnit": DataRef(objContext.props.businessunit)["Data"] } });
        }
    }, [objContext.props.businessunit]);
} 
