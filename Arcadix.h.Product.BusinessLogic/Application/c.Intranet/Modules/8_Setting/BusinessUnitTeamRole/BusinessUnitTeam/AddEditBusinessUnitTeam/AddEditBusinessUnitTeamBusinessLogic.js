...
export function mapStateToProps(state) {
    if (!global["mode"]) {
        return {
            object_cockpit_businessunit: DataRef(state.Entity, "object_cockpit_businessunit", true)
        };
    }
    else {
...
export function useInitializeRibbon(objContext) {
    ...
    useEffect(() => {
        ...
        ApplicationState.SetProperty("PopupTabData", { "addeditbusinessunitteam": arrTabData });
        ApplicationState.SetProperty("PopupRibbonData", { "addeditbusinessunitteam": arrRibbonData });
    }, [objContext.state]);
}
...
export function useInitializeTabs(objContext) {
    let arrContentData = [
        {//Group1
            "Text": objContext.props.Data.objTextResource["Team"],
            "Id": "NavId1",
            "Children": [
                {
                    "Text": objContext.props.Data.objTextResource["Team"],
    ...
    useEffect(() => {
        ApplicationState.SetProperty("PopupNavigationData", { "addeditbusinessunitteam": arrContentData });
    },
        [])
}
...
export function useInitializeData(objContext) {
    ...
    var objAddData = {
        "cIsDeleted": "N",
        "uBusinessUnitId": objContext.props.Data.strBusinessUnitId,
...
export const MultiSelectDropDownChange = (arrChangeData, props, objContext) => {
    console.log("arrChangeData ", arrChangeData)
    console.log("props ", props)
    console.log("objContext ", objContext)
    var objEditData = JSON.parse(JSON.stringify(objContext.state.objData));
    objEditData["userRole"] = arrChangeData;
...
export const AddMethod = (objContext, blnClose = false) => {
    ...
    let arrMetaData = [GetObjectHeader("vTeamName", objContext)];
    var blnIsValid = true;
    ...
    if (blnIsValid) {
        ...
        if (objContext.props.Data.blnIsAdd && !objContext.state.blnAddedOnce) {
            ...
            objArcadixFetchAndCacheData.Execute(arrParams, function (objReturn, cIsNewData) {
                if (cIsNewData) {
                    objContext.dispatch({ type: "SET_STATE_VALUES", payload: { "objBusinessUnitTeam": objReturn[0] } });
            ...
            if (objContext.state.blnAddedOnce) {
                var objEditData = JSON.parse(JSON.stringify(objContext.state.objData));
                objEditData["uTeamId"] = objContext.state.objBusinessUnitTeam["uTeamId"];
            }
            arrParams = [
                {
                    "URL": "API/Object/Cockpit/BusinessUnitTeam",
