...
export function GetInitialState(props) {
    ...
    if (
        Object_Framework_Services_TextResource.GetData("/c.Intranet/Modules/8_Setting/LogsAndException/Logs/HistoryLogs", props)
    ...
    return {
        ...
        objSearchFilters: {
            //dtFromDate: HistoryLogs_ModuleProcessor.GetPreviousDate(),
            dtToDate: HistoryLogs_ModuleProcessor.GetPreviousDate(),
            strType: "AllLogs",
...
export function useDataLoader(objContext) {
    useLayoutEffect(() => {
        objContext.HistoryLogs_ModuleProcessor.LoadInitialData(objContext);
    }, []);
}
...
export function useDataLoaded(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            objContext.HistoryLogs_ModuleProcessor.GetApplicationTypeData(objContext);
        }
        if (!objContext.state.isLoadComplete
            && Object_Framework_Services_TextResource.GetData("/c.Intranet/Modules/8_Setting/LogsAndException/Logs/HistoryLogs", objContext.props)
            ...
            objContext.props["Object_Framework_Services_TextResource;Id;" + objContext.props.JConfiguration.LanguageCultureInfo + "/c.Intranet/Modules/8_Setting/LogsAndException/Logs/HistoryLogs"],
...
export function useSetRibbonData(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            objContext.HistoryLogs_ModuleProcessor.SetRibbonData(objContext);
        }
    }, [objContext.state]);
