...
/**
 * @name AddEditWorkFlow_ModuleProcessor
 * @summary Class for Add/Edit ClientHostUrl module.
 */
class AddEditWorkFlow_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["uWorkflowId"] && objContext.state.objData["uWorkflowId"] != "") {
                Object_Cockpit_Workflow_Workflow.EditData({ "vEditData": [objContext.state.objData] }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "WorkFlowGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                Object_Cockpit_Workflow_Workflow.AddData({ "vAddData": objContext.state.objData }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    this.ResetGridSelection("WorkFlowGrid");
                    if (blnClose) {
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
...
export default AddEditWorkFlow_ModuleProcessor;