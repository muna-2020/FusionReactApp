...
/**
 * @name AddEditWorkFlowSatus_ModuleProcessor
 * @summary Class for Add/Edit Category module.
 */
class AddEditWorkFlowSatus_ModuleProcessor extends IntranetBase_ModuleProcessor {
    ...
    SaveData(objContext, blnClose = false) {
        ...
        if (!objValidationObject) {
            if (objContext.state.objData["uWorkflowStatusId"] && objContext.state.objData["uWorkflowStatusId"] != "") {
                ApplicationState.SetProperty("blnShowAnimation", true);
                Object_Cockpit_Workflow_WorkflowStatus.EditData({ "vEditData": [objContext.state.objData] }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    ApplicationState.SetProperty("blnShowAnimation", false);
                    if (blnClose) {
                        let objSelectedRows = ApplicationState.GetProperty("SelectedRows");
                        ApplicationState.SetProperty("SelectedRows", { ...objSelectedRows, "WorkFlowStatusGrid": [objReturn[0]] });
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
            } else {
                Object_Cockpit_Workflow_WorkflowStatus.AddData({ "vAddData": [objContext.state.objData] }, (objReturn, cIsNewData) => {
                    objContext.dispatch({ type: "SET_STATE", payload: { "objData": objReturn[0] } });
                    this.ResetGridSelection("WorkFlowStatusGrid");
                    if (blnClose) {
                        Popup.ClosePopup(objContext.props.Id);
                    }
                });
...
export default AddEditWorkFlowSatus_ModuleProcessor;