...
export function GetInitialState(props) {
    ...
    let arrPupilData = [];
    let objLearningJournalArchive_ModuleProcessor = new LearningJournalArchive_ModuleProcessor();
    let intSelectedSubjectId = objLearningJournalArchive_ModuleProcessor.GetUserpreferenceSubjectId();
...
export function useDataLoader(objContext) {
    useEffect(() => {
        objContext.LearningJournalArchive_ModuleProcessor.LoadInitialData(objContext);
    }, []);
}
...
export function useDataLoaded(objContext) {
    useEffect(() => {
        ...
        if (!objContext.state.isLoadComplete &&
            ...
            Object_Framework_Services_TextResource.GetData("/d.Extranet/3_Teacher/Modules/LearningJournalArchive", objContext.props) &&
        ...
        objContext.props["Object_Framework_Services_TextResource;Id;" + JConfiguration.LanguageCultureInfo + "/d.Extranet/3_Teacher/Modules/LearningJournalArchive"],
...
export function useDataLoaderForPupilAndTopic(objContext) {
    useEffect(() => {
        if (objContext.state.isLoadComplete) {
            ...
            (new ObjectQueue()).QueueAndExecute(objContext.LearningJournalArchive_ModuleProcessor.GetPupilDataParams(objContext),
                () => { ApplicationState.SetProperty("blnShowAnimation", false); }
            );
        }
